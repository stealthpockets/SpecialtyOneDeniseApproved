import { useState, useEffect } from 'react';
import { supabase } from '../lib/supabase';

interface ChartConfig {
  id: string;
  chart_id: string;
  title: string;
  type: 'line' | 'bar' | 'pie' | 'doughnut' | 'area';
  data: any;
  options: any;
}

interface UseChartsResult {
  charts: ChartConfig[];
  loading: boolean;
  error: string | null;
  getChartById: (chartId: string) => ChartConfig | undefined;
}

// Mock data for development/testing
const mockCharts: ChartConfig[] = [
  {
    id: '1',
    chart_id: 'market-cap-rates-2024',
    title: 'Average Cap Rates by Property Type - 2024',
    type: 'bar',
    data: {
      labels: ['Manufactured Housing', 'RV Parks', 'Self Storage', 'Multifamily', 'Industrial'],
      datasets: [{
        label: 'Average Cap Rate (%)',
        data: [6.8, 7.2, 5.9, 5.5, 6.1],
        backgroundColor: ['#8B5A3C', '#2D3A59', '#6B8E5A', '#8B5A3C99', '#2D3A5999'],
        borderColor: ['#8B5A3C', '#2D3A59', '#6B8E5A', '#8B5A3C', '#2D3A59'],
        borderWidth: 1
      }]
    },
    options: {
      responsive: true,
      maintainAspectRatio: false,
      plugins: {
        legend: {
          display: false
        }
      },
      scales: {
        y: {
          beginAtZero: true
        }
      }
    }
  },
  {
    id: '2',
    chart_id: 'transaction-volume-trend',
    title: 'Transaction Volume Trend - Last 12 Months',
    type: 'line',
    data: {
      labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
      datasets: [{
        label: 'Transaction Volume ($M)',
        data: [120, 135, 158, 142, 167, 189, 201, 185, 198, 224, 212, 195],
        borderColor: '#8B5A3C',
        backgroundColor: '#8B5A3C20',
        fill: true,
        tension: 0.4
      }]
    },
    options: {
      responsive: true,
      maintainAspectRatio: false,
      plugins: {
        legend: {
          display: false
        }
      },
      scales: {
        y: {
          beginAtZero: true
        }
      }
    }
  }
];

export const useCharts = (): UseChartsResult => {
  const [charts, setCharts] = useState<ChartConfig[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchCharts = async () => {
      try {
        setLoading(true);
        setError(null);

        // Try to fetch from Supabase first
        const { data, error: fetchError } = await supabase
          .from('chart_configs')
          .select('*')
          .order('created_at', { ascending: false });

        if (fetchError) {
          console.warn('Chart configs table not available, using mock data:', fetchError.message);
          // Fallback to mock data
          setCharts(mockCharts);
        } else {
          setCharts(data || mockCharts);
        }
      } catch (err) {
        console.warn('Error fetching charts, using mock data:', err);
        // Fallback to mock data
        setCharts(mockCharts);
      } finally {
        setLoading(false);
      }
    };

    fetchCharts();
  }, []);

  const getChartById = (chartId: string): ChartConfig | undefined => {
    return charts.find(chart => chart.chart_id === chartId);
  };

  return {
    charts,
    loading,
    error,
    getChartById
  };
};
